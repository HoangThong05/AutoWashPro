...
public class LoyaltyService {
        ...
        public void addPointsAfterPayment(Customer customer, int finalAmount) {
                ...
                // ← THÊM: Kiểm tra nâng hạng sau khi cộng điểm
        ...
        public LoyaltyResponse adjustPointsManually(ManualPointsRequest req, String staffEmail) {
                ...
                if (newPoints < 0)
                        throw new RuntimeException("Điểm không đủ để trừ");
        ...
        /* ===== KIỂM TRA VÀ NÂNG HẠNG TIER ===== */
        @Transactional
        public void checkAndUpgradeTier(Customer customer) {
                List<MemberTier> tiers = memberTierRepository
                                .findAllByOrderByPriorityDesc();

                for (MemberTier tier : tiers) {
                        if (customer.getLoyaltyPoints() >= tier.getMinPoints()
                                        && customer.getTotalWashes() >= tier.getMinWashes()
                                        && customer.getTotalSpend().compareTo(tier.getMinSpend()) >= 0) {

                                if (customer.getTier() == null ||
                                                !tier.getTierId().equals(customer.getTier().getTierId())) {

                                        TierHistory history = new TierHistory();
                                        history.setCustomer(customer);
                                        history.setOldTier(customer.getTier());
                                        history.setNewTier(tier);
                                        history.setChangeReason("AUTO_UPGRADE");
